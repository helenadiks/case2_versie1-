...
with tab3:
    ...
    if df_land[["renewables_share_elec", "co2_per_capita"]].dropna(how="all").empty:
        ...
        # Plotly gebruikt bij meerdere y-kolommen de kolomnamen zelf als
        # legenda-items (de "labels"-parameter hernoemt alleen de
        # as-/legendatitel, niet de items erin). Daarom hernoemen we de
        # kolommen hier eerst naar hun nette naam.
        df_land_nette_namen = df_land.rename(columns={
            "renewables_share_elec": LABELS["renewables_share_elec"],
            "co2_per_capita": LABELS["co2_per_capita"],
        })
        fig_line = px.line(
            df_land_nette_namen,
            x="year",
            y=[LABELS["renewables_share_elec"], LABELS["co2_per_capita"]],
